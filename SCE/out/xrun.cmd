...
Usage:
	xrun [options] files
	  ...
	  In addition to the dash options all xmverilog plus options can be used.
  ...
  -ams_svintf                SV Interface in AMS
  ...
  -bc_cf                     Do not mark array query functions as const expr
  -bc_dbits                  Revert new $bits checks to old behavior
  ...
  -clean                     Deletes previous xcelium.d directory if it exists
  ...
  -congruency_treat_ba_as_nba  Treat all blocking assignments as non blocking i
  -conn_status_log <arg>     Generates assertion status for connectivity
  -connect_specify_block     Connection are preserved in $setuphold call
  ...
  -delay_all_udp_ffs_inputs  If 1 or 2, rocketsim solves clk-data races in UD
  ...
  -deposit_value_change      Enable gate/UDP value change on deposit
  ...
  -efence                    Debug xmsim with Electric Fence.
  -efenceelab                Debug xmelab with Electric Fence
  ...
  -fault_test <dir>          Specify the directory for fault run database
  ...
  -forceelab                 Force xmelab to execute
  ...
  -gdb                       Run xmsim under gdb
  -gdbelab                   Run xmelab under gdb
  ...
  -helpxmverilog             Show the xmverilog form of the options
  -hier_dvs                  Enable hierarchical dynamic voltage supply
  -history                   Dislays the xrun history and exit
  -history_file <filename>   Sets xrun history file name
  ...
  -iusld                     Prefix `xmroot`/tools/lib path to LD_LIBRARY_PATH
  -iusldno                   Disable the -iusld option
  -ixcomargs <string>        Pass arguments to ixcom
  -jg                        Run JasperGold with xrun
  -jg_analyze_opts <arg>     Options for analyze command in JasperGold
  -jg_app <arg>              Specify the JasperGold App to be run
  -jg_clock <arg>            Specify clock for verification in JasperGold
  -jg_elab_opts <arg>        Options for elaborate command in JasperGold
  ...
  -libraries_skeleton        Different skeleton for each library
  ...
  -log_xmelab <logfile>      Place the xmelab output into specified logfile
  -log_xmprotect <logfile>   Place the xmprotect output into specified logfile
  -log_xmsc_run <logfile>    Place the xmsc_run output into specified logfile
  -log_xmsim <logfile>       Place xmsim output into the specified logfile
  -log_xmvhdl <logfile>      Place the xmvhdl output into specified logfile
  -log_xmvlog <logfile>      Place the xmvlog output into specified logfile
  ...
  -lps_sda_dont_touch_on     Enable UPF_dont_touch SDA commands
  ...
  -mce_abs_link              Use absolute paths when linking mce
  -mce_acc_file              List of interfaces to ACC
  -mce_access_file           access filename
  -mce_access_replication_limit  Limit the number of module replications
  -mce_access_suppress_errors  Suppress visibility errors if can't find signals
  -mce_access_suppress_internal_errors  Suppress internal visibility errors if can't fin
  -mce_allow_any_ram_in_loop  Allows RAMs with any read/write from within a lo
  -mce_allow_blast_rams      Allows small blast rams to be in ACC
  -mce_allow_mce_functions_to_nacc  Allow creating mce functions to nacc ids
  -mce_allow_mce_functions_to_part_select  Allow creating mce functions
                             (force/read/release
  -mce_allow_progressive_const_prop Allow progressive constant propagation
  -mce_args <string>         Pass arguments to mcebuild
  -mce_assertion_mode        Assertion mode; use -helpverbose for details
  -mce_auto_ignore_unused_interface_elements  Automatically ignore unused elements in
                             interfac
  -mce_auto_ignore_unused_interface_elements_exceptions_file  List of interfaces which
                             are exempt from automat
  -mce_bb_task_file          List of module:task pairs to be black-boxed
  -mce_congruency_file       Congruency configuration file
  -mce_copy_instead_of_link  Copy files from the build dir to the sim dir, in
  -mce_cpu_configuration     CPU configuration; use -helpverbose for details
  -mce_create_parent_scopes  Create skeleton for parent scopes and hierarchic
  -mce_create_write_repeater_pes  Create write repeater pes for for loops when pos
  -mce_debug_mode            Debug mode for Indago compilation, legal values:
  -mce_debug_no_cells        temporary flag, do not collect cell/user-library
  -mce_default_work_lib      Name of the work library
  -mce_deposit_mode          Replace $deposit with $mce_deposit_it
  -mce_deposit_only_into_wrapper_ports_containing_Q  mce_depositable_wrapper_out_ports -
                             ports with Q
  -mce_depositable_dangling_ports  Add logic allowing deposits to unconnected ports
  -mce_depositable_ffs       Allow dynamic deposits to all the FF's outputs
  -mce_depositable_output_ports_Q  Allow dynamic deposits to output ports with Q
  -mce_depositable_vars      Allow dynamic deposits to all used declared vars
  -mce_depositable_wrapper_out_ports  Allow dynamic deposits to ports which a seq PE
  -mce_detect_event_glitches  Detect event glitches; 0:none, 1:sequential, 2:a
  -mce_detect_udp_races      Detect UDP races
  -mce_display_task_mode     Display task mode; use -helpverbose for details
  -mce_dryrun                Execute a MCE dry run.
  -mce_enable_dpi_functions  Enable use of MCE DPI functions
  -mce_error_msg_ignore      Ignore some errors given by verific
  -mce_error_msg_warning     Lower some errors given by verific to warnings
  -mce_error_on_duplicate_modules  When a module is defined twice error out.
  -mce_expand_modules_to_instances  Replace reference to modules with reference to t
  -mce_expansion_split_max_size  Maximal size of a split for debug
  -mce_expansion_split_min_size  Minimal size of a split for debug
  -mce_expansion_split_threshold_size  Minimal size of a single pass elaboration split
  -mce_extract_hierarchy     Run mcebuild extract hierarchy and exit
  -mce_force_access_file     force-access filename
  -mce_force_aggregation     combine multiple force statements into one. 
  -mce_force_mode            Replace 'force a=0' with '$mce_force(''a'',0)'
  -mce_force_replh_file      Force replication of modules and descendants
  -mce_force_rhs_mode        RHS force mode; use -helpverbose for details
  -mce_gdb                   Launch gdb; use -helpverbose for details
  -mce_gdb_mode              gdb mode; use -helpverbose for details
  -mce_grid                  Parallel compilation on the grid
  -mce_grid_lsf_tracker_type  The LSF tracking to use when running on grid
  -mce_grid_max_lsf_launch_time  Max time to wait for grid command to launch
  -mce_grid_multiple_processes_per_job  Run multiple processes until all jobs are done
  -mce_hview                 Dump signals static info to hview
  -mce_ignore_empty_udp_errors  No error on white udps which become empty udps
  -mce_ignore_unused_interface_elements_file  List of interfaces in which unused
                             elements are 
  -mce_indago_probing        Optimize probing for Indago. When using this opt
  -mce_initial_block_to_value  Convert init block assignments to initial values
  -mce_loop_unroll_limit     Limit for loop unrolling
  -mce_looperror_abort_threshold  Threshold for loop detection error
  -mce_looperror_collisions_threshold  Collisions threshold for loop detection
  -mce_looperror_invocations_threshold  Threshold for loop detection
  -mce_looperror_report_invocations_count  Number of loop invocations to report
  -mce_lossy_udp_optimization  When set, UDP is optimized for speed, neglecting
  -mce_manual_defines_file   File with a list of definitions for the parser
  -mce_manual_init           Skeleton should explicitly initialize mce
  -mce_mark_all_depositable_vars_as_random_initable  All the signal marked ad
                             depositable will be mar
  -mce_mce_force_access_file  rock_force-access filename
  -mce_mode                  Simulation mode. Possible values: ['dft']
  -mce_nacc_file             List of modules to NACC
  -mce_nacc_interface_with_import  Nacc interface if it contains an import declarat
  -mce_nacc_interfaces_with_generate  Nacc interface if it contains a generate block
  -mce_nacc_module_with_strength_aware_pes Nacc module that contains strength aware pes
  -mce_nacc_module_with_strength_keywords  Nacc module that contains strength keywords
  -mce_nacc_nba_delay        Nacc NBAs that contain delay statement
  -mce_nacc_sva_dollar       Nacc sva assertions that contain ranges with '$'
  -mce_nacc_while_loops_with_non_const_cond  Nacc while loops with conditions that
                             contain a 
  -mce_nets_write_access     Host simulator will have write access to nets
  -mce_no_config_allow_escaped_name  Do not use -config_allow_escaped_names.
  -mce_no_grid_quote         Do not use quote on command sent through the gri
  -mce_no_logs               Disable all logging capabilities.
  -mce_overlay_nacc          Implement Nacc replication using overlay tree
  -mce_ovm_home              OVM package home directory for MCE.
  -mce_parse_tree_debug      1 - Create a reference design (dry-run); 2 - Cre
  -mce_pie                   Generate MCE performance report
  -mce_preserved_params_file  Contains a list of params that remain variables
  -mce_protected_mode        Support protectet module, legal values: 2, 1, 0
  -mce_qualify <file>        Report summary for mce qualification
  -mce_read_access_all       All variables will be visible
  -mce_read_access_all_debug All vars including mce internals will be visible
  -mce_read_access_file      read-access filename
  -mce_read_access_interfaces All interfaces variables will be visible
  -mce_read_cont_mode        Replace concat in RHS of nacc continuous assign
  -mce_read_mode             Automatically replace RHS with $mce_read'
  -mce_regenerate_logs_dir   Location of mce_regenerate logs directory (If no
  -mce_relax_double_quotes_string  Relax double quote; use -helpverbose for details
  -mce_replace_modules_flist  Provide an flists with files to replace.
  -mce_replace_random        Automatically replace $random with 0.
  -mce_run_stats_analyzer    Run the stats analyzer to get a compilation repo
  -mce_run_to_completion     Complete all compilations, even if some fail
  -mce_sanity_module_as_top  Do not instantiate mce_sanity as instance.
  -mce_sim_compatibility     Target host simulator
  -mce_sim_logs_dir <string> MCE Run Directory
  -mce_single_pass_rtl_elaboration  Run single pass RTL elaboration
  -mce_ske_compcnfg          Use -compcnfg to allow compilation of skeleton c
  -mce_skeleton_retain_binds  Instances coming from bind statements appear as 
  -mce_split_max_size        Maximal size of a split
  -mce_split_min_size        Minimal size of a split
  -mce_split_threshold_size  Minimal size of a single pass elaboration split
  -mce_splits_max_concurrency  Number of splits to be be compiled concurrently
  -mce_support_array_querying_functions  Accelerate the $size function for
                             non-dynamic-ty
  -mce_support_delays        implement gate delays in ACC
  -mce_support_genvar_in_interfaces  Don't Nacc interfaces with a genvar declaration
  -mce_support_nba_delays    Implement NBA delays in ACC
  -mce_support_unsigned_func  Accelerate the $unsigned function.
  -mce_top <instance path>   Full path to the accelerated top level
  -mce_tops_file             File listing the paths to the top of a hierarchy
  -mce_treat_mce_input_as_essential  All the mc-input (deposit/mc force) signals will
  -mce_use_stubs             Create stub if instantiated module not found
  -mce_uvm_home              UVM package home directory for MCE.
  -mce_vis_all               All user vars will be made visible for dumping
  -mce_vis_all_debug         All variables will be made visible for dumping
  -mce_vis_ff                All ACC FF variables will be dumped
  -mce_vis_file              waves dump filename
  -mce_vis_interfaces        Make all interfaces variables visible for dumpin
  -mce_vis_non_cell          Dump all interfaces vars of non-cell modules
  -mce_vis_packed_limit      Limit the size of packed MD arrays marked as Roc
  -mce_vis_unpacked_limit    Limit the size of unpacked MD arrays marked as R
  -mce_vmm_home              VMM package home directory for MCE.
  -mce_vmm_in_skeleton       Include VMM files in the output skeleton flist
  -mce_vpi_analysis          Enable signals analysis .csv
  -mce_warnings_as_errors    Treat warnings as errors
  -mce_waveform_file         waveform filename
  -mce_write_access_file     write-access filename
  ...
  -no_specify                Ignore delays in specify blocks
  ...
  -noelab                    Disable the invocation of xmelab
  ...
  -nohistory                 Disables xrun history and redo
  ...
  -norebuild                 Do not rebuild xmelab and xmsim
  ...
  -nosparsedefault           Dont set any default size for sparsearray
  ...
  -p2p                       Run next generation Incisiv-JG integration flow
  ...
  -prefix_xmsim <arg>        Add arguments to before xmsim execution
  -prep                      Run xrun in -prep mode
  ...
  -primlibdir <dir>          Specify directory of primary xrun command
  -primname <name>[@<dir>]   Specify the name of the primary xrun command
  ...
  -print_absolute_file_paths_in_skeleton  Whether the print of the linefiles of modules
                             in
  -print_hdl_precision       Prints VHDL timescale.
  -printspecs                Have xmsc_run print out the spec file
  ...
  -purecov                   Instrument xmsim code with purecov
  -purecovelab               Instrument xmelab with purecov
  -purify                    Instrument xmsim code with purify
  -purifyelab                Instrument xmelab with purify
  -Q                         Quiet mode, with banner and command line
  -q                         Suppress informational messages(i.e., Quiet mode)
  -quantify                  Instrument xmsim code with quantify
  -quantifyelab              Instrument xmelab with quantify
  ...
  -rebuild                   Rebuild the xmelab & xmsim that are to be used
  ...
  -rvf_nomirror              Disable xm_mirror generation for register flow
  -rvf_vpm                   Generate vsif for register checks distribution
  -s                         Load snapshot and go to the interactive prompt
  -save_run_history          Enables xrun history with -r and -R
  ...
  -snstubelab                Generate stubs via xmelab
  ...
  -split_delete_after_use    Attempt to trim memory after each split is dumpe
  -split_trim_memory         Attempt to trim memory after each split is dumpe
  ...
  -vhdl_ext <ext>            Override extensions for VHDL sources
  ...
  -vis_ingress               Dump all ingress signals to waves
  ...
  -wave_dumping_mode         Specifies how MCE treats wave dumping requests. 
  ...
  -xmdebug                   Equivalent to -access +r
  ...
  -xmelabfile                File for generated elab options from import
  ...
  -xmls_all                  Run xmls with the -all option
  -xmls_dep                  Run xmls with the -dep option
  -xmls_so                   Run xmls with the -source option
  -xmls_ss                   Run xmls with the -snapshot option
  -xmnote <arg>              Decreases the severity of a warning to a note
  -xmprotect_file <file_name> Pass arguments from a file to xmprotect
  ...
  -xmsimexe <exe>            Specify simulator with statically linked PLI
  -xmsimfile                 File for generated sim options from import
...
Example commands to invoke xrun
  xrun sio85.v
  xrun xor.v bot.vhd xor_verify.e
  xrun -f run.f
  xrun a.v b.v c.vhd -c
  xrun -R -input commands.tcl
  xrun -helpsubject specman
